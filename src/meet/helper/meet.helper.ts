...
export class MeetHelper {
  ...
  async getOldAttendancePhoto(meetingId: string, userId: string) {
    const oldPhoto = await this.prismaService.anggota_Rapat.findFirst({
      where: {
        rapatId: meetingId,
        userId: userId,
      },
  ...
  toParticipantsResponse(request, user) {
    return {
      id: user.id,
      nama: user.userData.nama,
      nip: user.userData.nip,
      kehadiran:
        user.anggotaRapat.length > 0 &&
        user.anggotaRapat.some((anggota) => anggota.kehadiran)
          ? 'Hadir'
          : 'Tidak Hadir',
      jabatan: {
